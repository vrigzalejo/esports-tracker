...
import { Database, Trash2, RefreshCw, CheckCircle, XCircle, AlertCircle, ChevronDown, ChevronUp } from 'lucide-react'
...
export default function RedisCacheStatus() {
  ...
  useEffect(() => {
    try {
      const savedState = localStorage.getItem('redis-cache-expanded')
      if (savedState !== null) {
  ...
  const toggleExpanded = () => {
    ...
    try {
      localStorage.setItem('redis-cache-expanded', JSON.stringify(newState))
    } catch (error) {
